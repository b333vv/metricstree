...
#### Phase 1: Foundational Local Checks (Checkstyle & PMD)
-   **Objective(s):** Provide the fastest possible feedback loop for the most common issues.
-   **Priority:** High

-   **Task 1.1: Step-by-Step Implementation Guide for Local Checks**
    -   **Rationale/Goal:** To provide a clear, actionable guide for setting up the initial local quality gates. This directly addresses how to connect and run Checkstyle and PMD.

    -   **Step 1.1.1: Add Gradle Plugins**
        -   **Action:** Modify the `build.gradle.kts` file to include the Checkstyle and PMD plugins. These plugins manage the tool dependencies, so you do not need to deploy anything manually.
    ...
    -   **Step 1.1.2: Configure Checkstyle**
        -   **Action:** Create a configuration directory and a Checkstyle rules file. We will start with the widely-used Google Java Style Guide.
        -   **1. Create Configuration File:** Create the file `config/checkstyle/checkstyle.xml`.
        -   **Code (`config/checkstyle/checkstyle.xml`):**
            ...
            <module name="Checker">
                ...
                <!-- Loads the Google Java Style Guide checks -->
