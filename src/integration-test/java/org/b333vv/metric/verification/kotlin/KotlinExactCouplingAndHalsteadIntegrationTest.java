...
public class KotlinExactCouplingAndHalsteadIntegrationTest extends LightJavaCodeInsightFixtureTestCase {
        ...
        public void testExactCouplingAndRfcValues() {
                ...
                // RFC expected per KotlinResponseForClassVisitor (declared functions + unique
                // call names/arity)
                assertEquals(1L, metricValue(repo.metric(MetricType.RFC)));
                // Includes constructor calls in property initializers (Repo() / Service()) as
                // call expressions
                assertEquals(4L, metricValue(service.metric(MetricType.RFC)));
                assertEquals(4L, metricValue(client.metric(MetricType.RFC)));

                // MPC expected per KotlinMessagePassingCouplingVisitor (count of call
                // expressions inside class bodies)
                ...
                // CBO expected per KotlinCouplingBetweenObjectsVisitor (unique external types
                // referenced)
                ...
                // Exact zero Halstead metrics expected for Repo: empty body => no
                // operators/operands
                ...
                // Length CHL = 4; Vocabulary CHVC = 4 (assignment '=' is not counted by current
                // visitor)
                ...
                assertEquals(Math.round(expectedDifficulty * 1000.0) / 1000.0,
                                Math.round(hAssign.metric(MetricType.CHD).getPsiValue().doubleValue() * 1000.0)
                                                / 1000.0);
                assertEquals(Math.round(expectedVolume * 1000.0) / 1000.0,
                                Math.round(hAssign.metric(MetricType.CHVL).getPsiValue().doubleValue() * 1000.0)
                                                / 1000.0);
                assertEquals(Math.round(expectedEffort * 1000.0) / 1000.0,
                                Math.round(hAssign.metric(MetricType.CHEF).getPsiValue().doubleValue() * 1000.0)
                                                / 1000.0);
                assertEquals(Math.round(expectedErrors * 1_000_000.0) / 1_000_000.0,
                                Math.round(hAssign.metric(MetricType.CHER).getPsiValue().doubleValue() * 1_000_000.0)
                                                / 1_000_000.0);

                // HalsteadLiteral: val s = "x"
                // Current visitor counts only the string literal as operand in class body
                // traversal
