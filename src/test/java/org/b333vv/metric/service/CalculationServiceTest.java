...
public class CalculationServiceTest {
        ...
        public void testCalculateProjectTreeWithCachedModel() {
                ...
                verify(metricsEventListener).projectMetricsTreeIsReady(treeModel, null);
        ...
        public void testGetOrBuildClassAndMethodModel_PsiStrategy() {
                ...
                when(psiStrategy.calculate(any(Project.class), any(ProgressIndicator.class), any()))
                                .thenReturn(projectElement);
        ...
        public void testModelBuildingDependencyChain() {
                ...
                try (MockedStatic<DumbService> mockedDumbService = mockStatic(DumbService.class);
                        ...
                        mockedAnalysisScope.when(() -> new AnalysisScope(any(Project.class)))
                                        .thenReturn(mockAnalysisScope);
                        ...
                        try (MockedStatic<DependenciesCalculator> mockedDependenciesCalculator = mockStatic(
                                ...
                                when(mockDependenciesCalculator.calculateDependencies())
                                                .thenReturn(mockDependenciesBuilder);
                                ...
                                MetricCalculationStrategy mockMetricCalculationStrategy = mock(
                                                MetricCalculationStrategy.class);
                                ProjectElement mockJavaProject = mock(ProjectElement.class);
                                when(mockMetricCalculationStrategy.calculate(any(), any(), any()))
                                                .thenReturn(mockJavaProject);
                                ...
                                PackageMetricsSetCalculator mockPackageMetricsSetCalculator = mock(
                                                PackageMetricsSetCalculator.class);
                                // calculate() returns void, so just verify interaction
                                mockedPackageMetricsSetCalculator
                                                .when(() -> new PackageMetricsSetCalculator(any(), any(), any()))
                                                .thenReturn(mockPackageMetricsSetCalculator);

                                ProjectMetricsSetCalculator mockProjectMetricsSetCalculator = mock(
                                                ProjectMetricsSetCalculator.class);
                                // calculate() returns void, so just verify interaction
                                mockedProjectMetricsSetCalculator
                                                .when(() -> new ProjectMetricsSetCalculator(any(), any(), any()))
                                ...
                                verify(mockCacheService, times(1)).putUserData(
                                                eq(CacheService.CLASS_AND_METHODS_METRICS),
                                                eq(mockJavaProject));
                                ...
                                verify(mockCacheService, times(1)).putUserData(eq(CacheService.PACKAGE_METRICS),
                                                any(ProjectElement.class)); // PackageMetricsSetCalculator modifies the
                                                                            // passed JavaProject
                                ...
                                verify(mockCacheService, times(1)).putUserData(eq(CacheService.PROJECT_METRICS),
                                                any(ProjectElement.class)); // ProjectMetricsSetCalculator modifies the
                                                                            // passed JavaProject
                        }
                }
        }
