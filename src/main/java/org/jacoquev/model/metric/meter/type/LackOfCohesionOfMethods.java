...
public class LackOfCohesionOfMethods implements Meter<JavaClass> {
    @Override
    public Set<Metric> meter(JavaClass javaClass) {
        PsiClass psiClass = javaClass.getPsiClass();
        long lackOfCohesionOfMethods = MetricsUtils.callInReadAction(() -> {
            long result = 0;
            if (TypeUtils.isConcrete(psiClass)) {
                result = getLackOfCohesionOfMethods(psiClass);
            }
            return result;
        });
        return ImmutableSet.of(
                Metric.of("LCOM", "Lack Of Cohesion Of Methods",
                        "/html/LackOfCohesionOfMethods.html", lackOfCohesionOfMethods)
        );
    }

    private long getLackOfCohesionOfMethods(PsiClass psiClass) {
