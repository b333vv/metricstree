...
/**
 * Visitor for calculating Coupling Intensity (CINT) metric for Kotlin functions
 * and constructors.
 * <p>
 * The CINT metric measures the degree of coupling between a function/method and
 * external types
 * by counting distinct foreign method calls, property accesses, and operator
 * invocations within
 * the function body. Higher CINT values indicate stronger coupling to external
 * dependencies.
 * </p>
 *
 * <h3>What is included in CINT calculation:</h3>
 * <ul>
 * <li><b>Qualified method calls:</b> Direct calls like {@code obj.method()},
 * safe calls {@code obj?.method()},
 * and chained calls {@code a.b.c()}</li>
 * <li><b>Property accesses:</b> Reading external properties via
 * {@code obj.property} or {@code obj?.property}</li>
 * <li><b>Infix function calls:</b> Calls using infix notation like
 * {@code obj add item}</li>
 * <li><b>Invoke operator calls:</b> Direct invocations of callable objects like
 * {@code funObj()}</li>
 * <li><b>Lambda invocations:</b> Calls to lambda parameters and function
 * references</li>
 * <li><b>Extension function calls:</b> Calls to extension functions on
 * qualified receivers</li>
 * <li><b>Collection operations:</b> Higher-order functions like
 * {@code list.map{}}, {@code filter{}}, etc.</li>
 * <li><b>Array access:</b> Element access via array/collection operators like
 * {@code arr[i]}</li>
 * <li><b>Delegation calls:</b> Method calls on delegated properties</li>
 * <li><b>Scope function calls:</b> Calls like {@code obj.let{}},
 * {@code apply{}}, {@code run{}}, etc.</li>
 * </ul>
 *
 * <h3>What is excluded from CINT calculation:</h3>
 * <ul>
 * <li><b>Self-calls:</b> Calls on {@code this} or {@code super} receivers</li>
 * <li><b>Unqualified calls:</b> Local function calls without explicit receiver
 * (e.g., {@code localFunc()})</li>
 * <li><b>Constructor calls:</b> Object instantiation expressions are not
 * counted as coupling</li>
 * <li><b>Nested function definitions:</b> Inner function declarations
 * themselves (only their calls)</li>
 * </ul>
 *
 * <h3>Distinctness criteria:</h3>
 * Each unique combination of (receiver expression + member name) is counted
 * only once.
 * For example, multiple calls to {@code obj.foo()} count as 1, but
 * {@code obj.foo()} and {@code obj.bar()}
...
public class KotlinCouplingIntensityVisitor extends KotlinMethodVisitor {
    ...
    public void visitPrimaryConstructor(@NotNull KtPrimaryConstructor constructor) {
        metric = Metric.of(CINT, 0);
    }

    @Override
    public void visitAnonymousInitializer(@NotNull KtAnonymousInitializer initializer) {
        metric = Metric.of(CINT, count(initializer, initializer.getBody()));
    }
    ...
    /**
     * Checks if an operator is a built-in Kotlin operator (not a custom infix
     * function).
     */
