 ...
 * 
 * <p>NOAV measures the total number of distinct variables accessed within a method body,
 * which serves as an indicator of method complexity and data coupling. Higher NOAV values
 * suggest that a method interacts with many different data sources, potentially indicating
 * poor cohesion or excessive complexity.
 * 
 * <h3>What is counted as an accessed variable:</h3>
 * <ul>
 *   <li><b>Method parameters:</b> {@code fun process(x: Int, y: String)} - both x and y are counted</li>
 *   <li><b>Local variables:</b> Variables declared within the function body with {@code val} or {@code var}</li>
 *   <li><b>Class properties:</b> Properties declared in the class (both own class and inherited)</li>
 *   <li><b>Primary constructor properties:</b> {@code class Person(val name: String)} - name is counted</li>
 *   <li><b>Loop variables:</b> Iterator variables in {@code for} loops</li>
 *   <li><b>Destructuring declarations:</b> {@code val (a, b) = pair} - both a and b are counted</li>
 *   <li><b>Lambda parameters:</b> {@code list.map { it + 1 }} - 'it' is counted</li>
 *   <li><b>Receiver variables:</b> {@code this}, {@code it}, extension function receivers</li>
 *   <li><b>When subject variable:</b> {@code when (val x = expr) { ... }} - x is counted</li>
 *   <li><b>Delegated properties:</b> {@code val lazy by lazy { ... }} - accessing 'lazy' is counted</li>
 *   <li><b>Backing fields:</b> {@code field} keyword in property getters/setters</li>
 *   <li><b>Qualified property access:</b> {@code obj.property} - property is counted if obj is a known variable</li>
 *   <li><b>Companion object properties:</b> Properties from companion objects of the containing class</li>
 * </ul>
 * 
 ...
 *   <li>Constants that are not variables (literals like {@code 42}, {@code "text"})</li>
 ...
 * <ul>
 *   <li><b>Low NOAV (1-5):</b> Simple method with few variable interactions - generally good</li>
 *   <li><b>Medium NOAV (6-10):</b> Moderate complexity - acceptable for most methods</li>
 *   <li><b>High NOAV (11+):</b> Complex method with many variable interactions - consider refactoring
 *       into smaller methods or reducing the number of variables</li>
 * </ul>
 * 
 * <h3>Metric significance:</h3>
 * <p>NOAV helps identify methods that:
 * <ul>
...
public class KotlinNumberOfAccessedVariablesVisitor extends KotlinMethodVisitor {
    ...
    public void visitNamedFunction(@NotNull KtNamedFunction function) {
        ...
        final Set<String> allowedNames = collectAllowedNames(function);
     ...
     * This includes parameters, local variables, class properties, and special variables.
     * 
    ...
    private Set<String> collectAllowedNames(@NotNull KtNamedFunction function) {
        ...
        KtParameter receiverParam = function.getReceiverTypeReference() != null ? 
            function.getValueParameters().stream().findFirst().orElse(null) : null;
        if (function.getReceiverTypeReference() != null) {
            allowed.add("this"); // Extension receiver is accessed via 'this'
        }
        
        // Add local properties and variables declared in function body
        KtExpression body = function.getBodyExpression();
        if (body != null) {
            body.accept(new KtTreeVisitorVoid() {
                @Override
