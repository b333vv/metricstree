...
/**
 * Visitor for calculating the Number Of Children (NOC) metric for Kotlin
 * classes.
 * 
 * <h2>Metric Definition</h2>
 * NOC measures the number of immediate subclasses of a class. This metric
 ...
 * The metric includes:
 * <ul>
 * <li><b>Direct subclasses</b>: Classes that explicitly extend the target class
 * using {@code : SuperClass}</li>
 * <li><b>Interface implementations</b>: Classes that implement the target
 * interface</li>
 * <li><b>Sealed class subclasses</b>: All permitted subclasses of sealed
 * classes (within same file/package restrictions)</li>
 * <li><b>Open class subclasses</b>: Subclasses of classes marked with
 * {@code open} keyword</li>
 * <li><b>Abstract class subclasses</b>: Implementations of abstract
 * classes</li>
 * <li><b>Mixed-language inheritors</b>: Both Kotlin and Java classes inheriting
 * from the target class</li>
 * </ul>
 * 
 * <h2>What is NOT Counted</h2>
 * The metric excludes:
 * <ul>
 * <li><b>Final classes</b>: Classes without {@code open}, {@code abstract}, or
 * {@code sealed} modifiers (returns 0 immediately)</li>
 * <li><b>Inner/nested classes</b>: Nested class relationships are not
 * inheritance</li>
 * <li><b>Companion objects</b>: Companion object relationships to their
 * containing class</li>
 * <li><b>Anonymous classes</b>: Anonymous object expressions are not counted as
 * children</li>
 * <li><b>Indirect descendants</b>: Only direct children are counted, not
 * grandchildren or further descendants</li>
 * <li><b>Classes outside project scope</b>: Library classes inheriting from
 * project classes are excluded for performance</li>
 * </ul>
 * 
 * <h2>Kotlin-Specific Considerations</h2>
 * <ul>
 * <li><b>Final by default</b>: Unlike Java, Kotlin classes cannot be inherited
 * unless explicitly marked</li>
 * <li><b>Sealed classes</b>: Subclasses are restricted and known at compile
 * time</li>
 * <li><b>Data classes</b>: Cannot be inherited (always final), so NOC is always
 * 0</li>
 * <li><b>Enum classes</b>: Cannot be inherited, NOC is always 0</li>
 * <li><b>Object declarations</b>: Singletons cannot be inherited, NOC is always
 * 0</li>
 * </ul>
 * 
 * <h2>Search Scope</h2>
 * The search is limited to the current project scope
 * ({@link GlobalSearchScope#projectScope(Project)}) for:
 * <ul>
 * <li>Performance optimization (avoids searching through all library
 * dependencies)</li>
 * <li>Relevance (external library inheritors are typically not under developer
 * control)</li>
...
public class KotlinNumberOfChildrenVisitor extends KotlinClassVisitor {
    ...
    private void compute(@NotNull KtElement element) {
        ...
        // using UAST for cross-language support (handles both Kotlin and Java
        // inheritors)
        UClass uClass = UastContextKt.toUElement(klass, UClass.class);
        if (uClass == null) {
            // UAST conversion failed - this can happen for malformed code or during
            // indexing
        ...
        try {
            ...
            // useDeepSearch = false ensures we only find direct children, not all
            // descendants
