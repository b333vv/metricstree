 ...
 * <p>
 * MPC measures the number of external method calls (messages sent) from a class to other classes.
 * This metric quantifies the coupling between a class and other classes through method invocations.
 * Higher MPC values indicate stronger coupling and potentially more maintenance effort.
 * </p>
 *
 * <h3>What is counted in MPC:</h3>
 * <ul>
 *   <li><b>Regular method calls:</b> Direct invocations of methods on external objects (e.g., {@code obj.method()})</li>
 *   <li><b>Property accessor calls:</b> Calls to getters and setters of external properties</li>
 *   <li><b>Extension function calls:</b> Calls to extension functions defined on external types</li>
 *   <li><b>Infix function calls:</b> Infix notation invocations (e.g., {@code a to b})</li>
 *   <li><b>Operator function calls:</b> Operator overloading invocations (e.g., {@code a + b}, {@code a[i]})</li>
 *   <li><b>Invoke operator calls:</b> Direct invocation on callable objects (e.g., {@code func()})</li>
 *   <li><b>Safe call expressions:</b> Calls using safe navigation operator (e.g., {@code obj?.method()})</li>
 *   <li><b>Scope function calls:</b> Standard library scope functions with lambdas (let, run, apply, also, with, use)</li>
 *   <li><b>Lambda invocations:</b> Calls within lambda expressions passed to external functions</li>
 *   <li><b>Callable references:</b> Method references used as function parameters (e.g., {@code ::method})</li>
 *   <li><b>Companion object calls:</b> Calls to methods in companion objects of external classes</li>
 *   <li><b>Top-level function calls:</b> Calls to functions defined at package level in external packages</li>
 *   <li><b>Constructor calls:</b> Object instantiation of external classes (e.g., {@code ExternalClass()})</li>
 *   <li><b>Super calls:</b> Calls to parent class methods using {@code super} keyword</li>
 *   <li><b>Nested and inner class calls:</b> Calls to methods in nested/inner classes of external types</li>
 * </ul>
 *
 * <h3>What is excluded from MPC:</h3>
 * <ul>
 *   <li><b>Internal method calls:</b> Calls to methods within the same class (including this.method())</li>
 *   <li><b>Standard library calls:</b> Calls to Java standard library (java.*, javax.*) and Kotlin standard library (kotlin.*)</li>
 *   <li><b>Private function calls:</b> Calls to private functions within the same class</li>
 *   <li><b>Local function calls:</b> Calls to functions defined locally within the same method</li>
 *   <li><b>Type references:</b> Simple type usage without method invocation</li>
 *   <li><b>Field access:</b> Direct field access without method invocation (unless property getter is called)</li>
 * </ul>
 *
 * <h3>Counting rules:</h3>
 * <ul>
 *   <li>Each method call is counted separately, even if the same method is called multiple times</li>
 *   <li>Chained calls are counted individually (e.g., {@code obj.m1().m2()} counts as 2 calls)</li>
 *   <li>Calls in all class members are included: functions, properties, init blocks, constructors</li>
 *   <li>Calls in nested lambdas and anonymous functions are included</li>
 *   <li>Unresolved calls (where type resolution fails) are excluded for accuracy</li>
 * </ul>
 *
 * <h3>Result:</h3>
 ...
 *   <li><b>Medium MPC (11-30):</b> Moderate coupling, acceptable for most classes</li>
 *   <li><b>High MPC (31+):</b> Strong coupling, may indicate god class or poor separation of concerns</li>
 * </ul>
 *
 * <h3>Limitations:</h3>
 * <ul>
 *   <li>Requires successful PSI type resolution - unresolved references are not counted</li>
...
public class KotlinMessagePassingCouplingVisitor extends KotlinClassVisitor {
        ...
        if (body != null) {
            body.accept(new KtTreeVisitorVoid() {
                ...
                public void visitBinaryExpression(@NotNull KtBinaryExpression expression) {
                    ...
                    if (isOperatorOrInfixCall(expression, selfName)) {
                        calls[0]++;
                    }
                ...
                public void visitUnaryExpression(@NotNull KtUnaryExpression expression) {
                    ...
                    if (isExternalUnaryOperator(expression, selfName)) {
                        calls[0]++;
                    }
                ...
                public void visitArrayAccessExpression(@NotNull KtArrayAccessExpression expression) {
                    ...
                    if (isExternalArrayAccess(expression, selfName)) {
                        calls[0]++;
                    }
                ...
                public void visitDotQualifiedExpression(@NotNull KtDotQualifiedExpression expression) {
                    ...
                    if (isExternalPropertyAccess(expression, selfName)) {
                        calls[0]++;
                    }
                ...
                public void visitSafeQualifiedExpression(@NotNull KtSafeQualifiedExpression expression) {
                    ...
                    if (isExternalSafeCall(expression, selfName)) {
                        calls[0]++;
                    }
                ...
                public void visitCallableReferenceExpression(@NotNull KtCallableReferenceExpression expression) {
                    ...
                    if (isExternalCallableReference(expression, selfName)) {
                        calls[0]++;
                    }
    ...
    /**
     * Determines if a call expression represents an external method call.
     *
     * @param expression the call expression to check
     * @param sourceClass the class being analyzed
     * @param selfName the fully qualified name of the source class
     * @return true if this is an external call that should be counted
     */
    ...
    private boolean isOperatorOrInfixCall(KtBinaryExpression expression, String selfName) {
        try {
            // Check if this is a simple assignment or comparison that doesn't involve external calls
    ...
    /**
     * Checks if a qualified expression represents an external property access (getter call).
     */
    ...
    private boolean isExternalSafeCall(KtSafeQualifiedExpression expression, String selfName) {
        try {
            ...
            if (selector instanceof KtCallExpression) {
                return isExternalCall((KtCallExpression) selector, null, selfName);
    ...
    /**
     * Checks if the resolved element is a local function defined within the current scope.
     */
    ...
    /**
     * Gets the fully qualified name of the container class/object for a given element.
     */
