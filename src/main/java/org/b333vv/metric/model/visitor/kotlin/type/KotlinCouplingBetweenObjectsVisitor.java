...
/**
 * Visitor for calculating Coupling Between Objects (CBO) metric for Kotlin
 * classes.
 * <p>
 * The CBO metric counts the number of unique external types that a class is
 * coupled to.
 * A class is coupled to another class if it uses that class in any way
 ...
 * <ul>
 * <li><b>Supertypes:</b> All direct parent classes and implemented interfaces
 * from superTypeListEntries</li>
 * <li><b>Primary constructor parameters:</b> Types of all property parameters
 * (val/var) in primary constructor</li>
 * <li><b>Secondary constructors:</b> Parameter types in all secondary
 * constructors</li>
 * <li><b>Properties:</b> Types of all properties (val/var) declared in class
 * body</li>
 * <li><b>Property delegates:</b> Types used in property delegation (by
 * keyword)</li>
 * <li><b>Functions:</b> Receiver types, parameter types, and return types of
 * all functions</li>
 * <li><b>Function bodies:</b> Types of local variables and expressions within
 * function implementations</li>
 * <li><b>Lambda parameters:</b> Types used in lambda expressions and function
 * literals</li>
 * <li><b>Type aliases:</b> Resolved types when type aliases are used</li>
 * <li><b>Annotations:</b> Types of annotations applied to class, properties,
 * functions, and parameters</li>
 * <li><b>Class delegates:</b> Types used in class delegation (interfaces
 * delegated to another object)</li>
 * <li><b>Init blocks:</b> Types referenced in class initialization blocks</li>
 * <li><b>Companion objects:</b> Types used within companion object
 * declarations</li>
 * <li><b>Nested classes:</b> For nested classes, CBO is calculated separately
 * and does not affect outer class</li>
 * <li><b>Generic type arguments:</b> All type arguments used in generic type
 * declarations (e.g., List&lt;String&gt; counts String)</li>
 * </ul>
 *
 * <h3>What is excluded from CBO calculation:</h3>
 * <ul>
 * <li><b>Standard library types:</b> Classes from kotlin.*, java.*, and javax.*
 * packages</li>
 * <li><b>Self-references:</b> References to the class itself</li>
 * <li><b>Primitive types:</b> Built-in Kotlin primitive types (Int, String,
 * Boolean, etc.)</li>
 * <li><b>Type parameters:</b> Generic type parameter names (T, E, K, V) without
 * bounds</li>
 * </ul>
 *
 * <h3>De-duplication:</h3>
 * Each unique external type is counted only once, regardless of how many times
 * it appears in the class.
 *
 * <h3>Result:</h3>
 ...
 * <h3>Limitations:</h3>
 * <ul>
 * <li>Type resolution depends on PSI and may fail for unresolved
 * references</li>
...
public class KotlinCouplingBetweenObjectsVisitor extends KotlinClassVisitor {
    ...
    private void compute(@NotNull KtElement element) {
        ...
        // 1. Supertypes (inheritance and interfaces) - only for Classes/Objects
        if (element instanceof KtClassOrObject) {
            List<KtSuperTypeListEntry> superTypes = ((KtClassOrObject) element).getSuperTypeListEntries();
            for (KtSuperTypeListEntry st : superTypes) {
                addTypesFromTypeRef(types, st.getTypeReference(), selfName);
                KtDelegatedSuperTypeEntry delegated = st instanceof KtDelegatedSuperTypeEntry
                        ? (KtDelegatedSuperTypeEntry) st
                        : null;
        ...
        // 2. Annotations on the element itself
        if (element instanceof KtAnnotated) {
            collectAnnotationTypes(types, ((KtAnnotated) element).getAnnotationEntries(), selfName);
        }

        // 3. Primary constructor (only for classes)
        ...
        // 4. Body elements or file declarations
    ...
    /**
     * Processes a property declaration, including its type, delegate, initializer,
     * and annotations.
     */
    ...
    /**
     * Processes a function declaration, including receiver, parameters, return
     * type, and body.
     */
    ...
    private void processNestedObject(Set<String> types, KtObjectDeclaration obj, String selfName) {
        // For companion objects, include types they reference as part of outer class
        // coupling
        if (obj.isCompanion()) {
            // Supertypes
        ...
        // For named objects, we could either include or exclude them - current approach
        // excludes
        // their internal coupling from outer class, which is more conservative
    }

    /**
     * Scans an expression for type references, including local variables, casts,
    ...
    /**
     * Extracts and adds all user types from a type reference, including nested
     * generic arguments.
     */
