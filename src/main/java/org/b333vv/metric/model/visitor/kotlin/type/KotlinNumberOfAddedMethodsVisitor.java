 ...
 * <p>
 * Calculates the number of operations (methods and accessors) added by a Kotlin
 * class that do not
 * override methods from supertypes. This metric helps assess the new
 ...
 * <p>
 * NOAM counts all methods and method-like operations defined in the class that
 * represent new
 * functionality rather than implementations of inherited contracts. This
 ...
 * <ul>
 * <li>All named functions declared in the class body that do not have the
 * {@code override} modifier</li>
 * <li>Includes functions with special modifiers: {@code inline}, {@code infix},
 * {@code operator}, {@code suspend}</li>
 ...
 * {@code val} or {@code var} in the primary constructor</li>
 * <li><b>Body properties:</b> Properties declared in the class body (excluding
 * companion object properties)</li>
 ...
 * </ul>
 * </li>
 * <li><b>Custom accessors:</b> Properties with explicit getter/setter bodies
 * are counted the same way</li>
 * <li><b>Delegated properties:</b> Properties using {@code by} delegation are
 * counted as standard properties</li>
 * <li>Properties with {@code override} modifier are excluded</li>
 * <li>Example: {@code var name: String} counts as 2 added methods (getName,
 * setName)</li>
 * </ul>
 *
 * <h3>3. Data Class Generated Methods</h3>
 * <p>
 * Data classes automatically generate several methods. The following are
 * counted as added methods:
 * </p>
 * <ul>
 * <li><b>copy() method:</b> Creates a copy of the object with optional
 * parameter modifications (1 method)</li>
 * <li><b>componentN() methods:</b> Destructuring declarations, one per primary
 * constructor parameter</li>
 * </ul>
 * <p>
 * <b>Note:</b> The following data class methods are <em>not</em> counted
 * because they override methods
 ...
 * <ul>
 * <li>All functions declared in companion objects (Kotlin's equivalent of
 * static methods)</li>
 * <li>These are counted as added methods since they're part of the class's
 * API</li>
 * <li>Companion object properties and their accessors are <em>not</em> counted
 * in NOAM</li>
 * <li>Example: {@code companion object { fun create(): MyClass }} counts as 1
 * added method</li>
 * </ul>
 *
 * <h3>5. Operator Overloading</h3>
 * <ul>
 * <li>Functions marked with {@code operator} modifier for operator
 * overloading</li>
 * <li>Examples: {@code operator fun plus()}, {@code operator fun get()},
 * {@code operator fun invoke()}</li>
 ...
 * <li>Data class methods that override {@code Any}: equals, hashCode,
 * toString</li>
 * </ul>
 *
 * <h2>Examples</h2>
 ...
 * <li>Abstract classes and interfaces are processed the same way as concrete
 * classes</li>
 * <li>The {@code override} keyword is the key discriminator for excluding
 * inherited methods</li>
 * <li>Delegated properties are treated as regular properties for counting
 * purposes</li>
 * </ul>
 *
...
public class KotlinNumberOfAddedMethodsVisitor extends KotlinClassVisitor {
    ...
    private void compute(@NotNull KtElement element) {
        ...
        added += countExplicitFunctions(element);

        // 2. Count property accessors (getters/setters)
        added += countPropertyAccessors(element);

        // 3. Count companion object methods
        if (element instanceof KtClass) {
            added += countCompanionObjectMethods((KtClass) element);
        }

        // 4. Count data class generated methods
        if (element instanceof KtClass && ((KtClass) element).isData()) {
            added += countDataClassAddedMethods((KtClass) element);
        }
     ...
     * <p>
     * Includes functions with any modifiers (inline, infix, operator, suspend)
     * except override.
     * </p>
     *
     ...
     */
    private int countExplicitFunctions(@NotNull KtElement element) {
        int count = 0;
    ...
    /**
     * Counts property accessors (getters and setters) for properties that are not
     * overrides.
     * <p>
     ...
     * </ul>
     * Properties in companion objects are excluded as they are not instance
     * members.
     * </p>
     *
     * @param klass the class to analyze
     ...
     */
    private int countPropertyAccessors(@NotNull KtElement element) {
        int count = 0;

        // Count primary constructor properties (only for classes)
        if (element instanceof KtClass) {
     ...
     * <li><b>equals(), hashCode(), toString():</b> NOT counted (override Any
     * methods)</li>
     * </ul>
     * <p>
