...
public class JavaParserHalsteadClassVisitor extends JavaParserClassVisitor {
    ...
    public void visit(ClassOrInterfaceDeclaration n, Consumer<Metric> collector) {
        ...
        if (n1 > 0 && n2 > 0 && N1 > 0 && N2 > 0) {
            collector.accept(createMetric(MetricType.CHVL, (double) (N1 + N2) * (Math.log(n1 + n2) / Math.log(2))));
            collector.accept(createMetric(MetricType.CHD, (double) n1 / 2 * N2 / n2));
            collector.accept(createMetric(MetricType.CHL, (double) 2 / n1 * n2 / N2));
            collector.accept(createMetric(MetricType.CHEF, (double) (n1 * N2 * (N1 + N2) * Math.log(n1 + n2)) / (2 * n2)));
            collector.accept(createMetric(MetricType.CHVC, (double) (N1 + N2) * Math.log(n1 + n2) / Math.log(2)));
            collector.accept(createMetric(MetricType.CHER, (double) ((n1 * N2 * (N1 + N2) * Math.log(n1 + n2)) / (2 * n2)) / 3000));
        }
    }
