...
public class JavaParserNumberOfAddedMethodsVisitor extends JavaParserClassVisitor {
    ...
    public void visit(ClassOrInterfaceDeclaration n, Consumer<Metric> collector) {
        ...
        long noam = 0;
        if (!n.isInterface()) {
            noam = n.getMethods().stream()
                    .filter(m -> !m.getAnnotationByName("Override").isPresent())
                    .count();
        }
        Metric metric = Metric.of(MetricType.NOAM, Value.of(noam));
